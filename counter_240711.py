...
vycistena_slova = []                            # založím prázdný list

for slovo in TEXT.split():                      # pro každé slovo z listu (vytvořený pomocí metody split, která rozdělí string TEXT na slova a podle defaultně mezer) 
    ciste_slovo = slovo.strip('.,?:!<>()')      # na všechna slova púoužiji metodu strip a zbavím je znaků v závorce
    vycistena_slova.append(ciste_slovo.lower())   # do listu vycistena_slova vložím "cista_slova" zapsaná pouze malými písmeny
...
{'affronting': 1, 'imprudence': 1, 'do': 2, 'he': 8, 'everything': 1, 'sex': 2, 'lasted': 1, 'dinner': 1, 'wanted': 1, 'indeed': 1, ...
 '''
vyskyt_slov = {}                    # prázdný slovník

for slovo in vycistena_slova:       # pro každé slovo z listu vyčištěná slova 
    if slovo not in vyskyt_slov:    # když slovo není ve slovníku vyskyt_slov
        vyskyt_slov[slovo] = 1      # zapiš do vyskyt slov klíč slovo s hodnotou 1
    else:
        vyskyt_slov[slovo] =vyskyt_slov[slovo] + 1  # jinak (když je ve slovníku) přičti k hodnotě 1
...
'''
nejcastejsi_hodnoty = sorted(list(vyskyt_slov.values()), reverse=True)[:9]
